...
int configVlan_ESW(PSWFabHALArg args, int numArgs, BOOL up) {
	...
	//brlan1 case check for iot
	if (101 == args[0].vidParams.vid)
	{
		syscfg_init();
        memset(iot_enabled, 0, 20);
        rc = syscfg_get(NULL, "X_RDKCENTRAL-COM_ENABLEIOT", iot_enabled, sizeof(iot_enabled));
