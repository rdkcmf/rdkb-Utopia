...
static int add_members(PL2Net network, PMember interfaceBuf, int numMembers) {
    ...
    //add local ready vlan interfaces
//     for (i = 0; i < numMembers; ++i) {
//         if (interfaceBuf[i].bReady && interfaceBuf[i].interface.bLocal)
//             nethelper_bridgeAddIf(network->name, interfaceBuf[i].interface.name);
//     }
