...
#ifndef HNAPD_SIMULATOR_STATE_FILE
#define HNAPD_SIMULATOR_STATE_FILE "simulator.ds"
#endif


/* Server device context module node */
typedef struct _HNAPD_ServerContext_Module
{
    HDK_SRV_MODULE_HANDLE hModule;
    HDK_SRV_ModuleContext moduleCtx;
    struct _HNAPD_ServerContext_Module* pNext;
} HNAPD_ServerContext_Module;


/* Server (and module) device context */
typedef struct _HNAPD_ServerContext
{
    HDK_SRV_Simulator_ModuleCtx simulatorCtx;
    HNAPD_ServerContext_Module* pModulesHead;
} HNAPD_ServerContext;


/* Initialize the server device context.  Returns non-zero for success, zero otherwise. */
void* HNAPD_ServerContext_Init()
{
    /* Malloc an simulator context */
    HNAPD_ServerContext* pServerCtx = (HNAPD_ServerContext*)malloc(sizeof(HNAPD_ServerContext));
    if (pServerCtx)
    {
        FILE* pfh;
        ...
        pfh = fopen(HNAPD_SIMULATOR_STATE_FILE, "rb");
...
void HNAPD_ServerContext_Free(void* pServerCtx, int fCommit)
{
    HNAPD_ServerContext_Module* pModule;

    /* Commit simulator state changes? */
    if (fCommit)
    {
        /* Open the simulator state file */
        FILE* pfh = fopen(HNAPD_SIMULATOR_STATE_FILE, "wb");
        if (pfh)
        {
            /* Write the simulator context */
            HDK_SRV_Simulator_Write(&((HNAPD_ServerContext*)pServerCtx)->simulatorCtx, HDK_XML_OutputStream_File, pfh);
    ...
    /* Free the simulator context */
    HDK_SRV_Simulator_Free(&((HNAPD_ServerContext*)pServerCtx)->simulatorCtx);

    /* Unload server modules */
    for (pModule = ((HNAPD_ServerContext*)pServerCtx)->pModulesHead; pModule; )
    {
        HNAPD_ServerContext_Module* pNext = pModule->pNext;
...
HDK_SRV_ModuleContext** HNAPD_ServerModules_Init(void* pServerCtx)
{
...
#endif
    {
...
#endif
        {
            ...
            if (hModule)
            {
                ...
                HNAPD_ServerContext_Module* pNew = (HNAPD_ServerContext_Module*)malloc(sizeof(HNAPD_ServerContext_Module));
                if (pNew)
                {
                    nModules++;
                    ...
                    pNew->pNext = ((HNAPD_ServerContext*)pServerCtx)->pModulesHead;
                    ((HNAPD_ServerContext*)pServerCtx)->pModulesHead = pNew;
                }
                else
                {
    ...
    if (ppModuleCtxs)
    {
        ...
        HNAPD_ServerContext_Module* pModule;
        for (pModule = ((HNAPD_ServerContext*)pServerCtx)->pModulesHead; pModule; pModule = pModule->pNext)
        {
...
void HNAPD_ServerModules_Free(HDK_SRV_ModuleContext** ppModuleContexts)
{
...
int HNAPD_Authenticate(void* pServerCtx, const char* pszUser, const char* pszPassword)
{
    /* Get the device user and password */
    HDK_SRV_Simulator_ModuleCtx* pSimulatorCtx = &((HNAPD_ServerContext*)pServerCtx)->simulatorCtx;
...
void HNAPD_HNAPResult(void* pServerCtx, HDK_XML_Struct* pOutput,
