...
#   elif [ "started" = "$1" -a "started" = "$CURRENT_LAN_STATE" ] ; then
	 ...
	 if [ "$XDNS_ENABLE" = "true" ]; then
		 $SERVER -u nobody -q --clear-on-reload --bind-interfaces --add-mac --add-cpe-id=abcdefgh -P 4096 -C $DHCP_CONF  #--enable-dbus
	 else
   ...
   if [ "0" = "$SYSCFG_dhcp_server_enabled" ] ; then
     if [ "$XDNS_ENABLE" = "true" ]; then
      ...
      # that lease is not found in the dnsmasq.leases file
      	 if [ "$XDNS_ENABLE" = "true" ]; then
		$SERVER -u nobody -q --clear-on-reload --bind-interfaces --add-mac --add-cpe-id=abcdefgh --dhcp-authoritative -P 4096 -C $DHCP_CONF  #--enable-dbus
	 else
